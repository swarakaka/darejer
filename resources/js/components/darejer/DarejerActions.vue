...
function buttonClasses(action: DarejerAction): string {
  const base =
    'inline-flex items-center gap-1.5 text-sm font-medium h-8 px-3 rounded-md border shadow-xs transition-colors'
  if (action.type === 'Save') {
    return `${base} bg-brand-600 hover:bg-brand-700 text-white border-transparent shadow-sm`
  }
  if (action.variant === 'destructive') {
    return `${base} bg-card hover:bg-danger-50 hover:border-danger-300 text-danger-700 border-danger-200`
  }
  return `${base} bg-card hover:bg-paper-100 hover:border-paper-400 text-ink-700 border-paper-300`
}
...
<template>
  <TooltipProvider :delay-duration="0">
    <div class="flex flex-wrap items-center gap-1.5" :class="placementClass">
      <template v-for="action in visibleActions" :key="action.label">
        ...
        <DropdownMenu v-else-if="action.type === 'Dropdown'">
          <DropdownMenuTrigger as-child>
            <button type="button" :disabled="action.disabled" :class="buttonClasses(action)">
              <component :is="resolveIcon(action.icon)" v-if="action.icon" class="h-3.5 w-3.5" />
              {{ __(action.label) }}
          ...
          <DropdownMenuContent align="end">
            <template v-for="item in action.items ?? []" :key="item.label">
              ...
              >
                <component :is="resolveIcon(item.icon)" v-if="item.icon" class="h-3.5 w-3.5" />
                {{ __(item.label) }}
              </DropdownMenuItem>
            </template>
          </DropdownMenuContent>
