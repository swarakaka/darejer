...
const { load, http } = useDataUrl<Option>(props.component.dataUrl as string | undefined, {
  perPage: 50,
  combobox: true,
...
async function fetchOptions(reset = false): Promise<void> {
  ...
  if (staticOptions.value) {
    options.value = staticOptions.value.filter(
      (o) => !search.value || o.label.toLowerCase().includes(search.value.toLowerCase()),
    )
    hasMore.value = false
    return
  }
...
function labelFor(value: string): string {
  return labelMap.value[value] ?? options.value.find((o) => o.value === value)?.label ?? value
}

const selectedLabels = computed(() => selected.value.map((v) => labelFor(v)))
...
const isClearable = computed(() => props.component.clearable !== false && !props.component.disabled)

const canShowClear = computed(() => isClearable.value && selected.value.length > 0)
  ...
  <FieldWrapper :component="component" :record="record" :errors="errors" :form-data="formData">
    <template #default="{ hasError }">
      <Popover :open="open" @update:open="open = $event">
        <div class="relative">
          <PopoverTrigger as-child>
            ...
            >
              ...
              <div v-else class="flex flex-wrap items-center gap-1 overflow-hidden">
                <Badge
        ...
        <PopoverContent class="w-(--reka-popper-anchor-width) min-w-56 p-0" align="start">
          <Command :should-filter="false">
            <CommandInput
              v-if="component.searchable !== false"
              v-model="search"
              :placeholder="__('Search…')"
              class="h-8 border-none text-sm"
            />

            <CommandList class="max-h-56 overflow-y-auto" @scroll.passive="onScroll">
              ...
              <CommandGroup>
                ...
                <div
                  ...
                  class="flex h-7 items-center justify-center text-xs text-slate-400"
                >
