...
const containerClass = computed(() => {
  ...
  if (style.value === 'pills')
    return 'flex gap-1 p-1 bg-paper-75 rounded-md border border-paper-200'
...
<template>
  <div :class="containerClass" class="col-span-full">
    <template v-for="item in items" :key="item.label">
      <Link v-if="item.url" :href="item.url" :class="itemClass(item)">
        {{ item.label }}
      </Link>
      <button v-else type="button" :class="itemClass(item)">
        {{ item.label }}
      </button>
    </template>
  </div>
