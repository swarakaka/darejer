...
function parseInitial(): Row[] {
  const raw = (props.formData ?? props.record)[props.component.name] ?? props.component.default ?? {}
...
function emitValue() {
  const obj = Object.fromEntries(rows.value.filter((r) => r.key.trim() !== '').map((r) => [r.key.trim(), r.value]))
...
<template>
  <FieldWrapper :component="component" :record="record" :errors="errors" :form-data="formData">
    <template #default="{ hasError }">
      <div class="overflow-hidden rounded border" :class="hasError ? 'border-danger-600' : 'border-slate-200'">
        <!-- Header -->
        <div class="bg-slate-75 grid grid-cols-[1fr_1fr_2rem] border-b border-slate-200">
        ...
        >
          ...
          <div class="flex items-center justify-center border-s border-slate-200">
            <button
              type="button"
              :disabled="component.disabled as boolean"
              class="hover:text-danger-600 flex h-8 w-8 items-center justify-center text-slate-300 transition-colors duration-100 disabled:opacity-40"
        ...
        <!-- Add row button -->
        <div v-if="!component.disabled" class="bg-slate-75 flex items-center border-t border-slate-200 px-2 py-1.5">
          <button
            type="button"
            class="text-brand-600 hover:text-brand-700 flex items-center gap-1.5 text-xs font-medium transition-colors duration-100"
            @click="addRow"
          >
            <Plus class="h-3 w-3" />
            {{ __('Add row') }}
