...
export function useLanguages() {
  ...
  const languages = computed(() => page.props.darejer?.languages ?? ['en'])

  const defaultLanguage = computed(() => page.props.darejer?.default_language ?? languages.value[0])
  ...
  const currentLocale = computed(() => page.props.darejer?.locale ?? defaultLanguage.value)
