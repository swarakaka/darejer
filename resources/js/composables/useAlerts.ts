...
export function useAlerts() {
  ...
  function refreshCount(): Promise<void> {
    return countHttp
      .get(route('darejer.alerts.count').toString(), {
        onSuccess: (data) => {
  ...
  function loadList(): Promise<void> {
    loading.value = true
    return listHttp
      .get(route('darejer.alerts.index').toString(), {
        onSuccess: (data) => {
  ...
  function markRead(id: number): Promise<void> {
    return ackHttp
      .post(route('darejer.alerts.read', { id }).toString(), {
        onSuccess: (data) => {
  ...
  function destroy(id: number): Promise<void> {
    return ackHttp
      .delete(route('darejer.alerts.destroy', { id }).toString(), {
        onSuccess: () => {
  ...
  function clearAll(): Promise<void> {
    return ackHttp
      .delete(route('darejer.alerts.clear').toString(), {
        onSuccess: () => {
          items.value = []
          unreadCount.value = 0
        },
  ...
  function subscribe(): void {
    ...
    echo.private(`darejer.alerts.${id}`).listen('.alert.created', (payload) => {
