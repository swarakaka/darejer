...
export function formatPhpDate(date: Date, format: string): string {
    return format.replace(TOKEN_RE, (token) => {
        ...
        switch (token) {
            ...
            case 'F': return MONTHS_FULL[date.getMonth()]
            case 'M': return MONTHS_SHORT[date.getMonth()]
            case 'l': return DAYS_FULL[date.getDay()]
            case 'D': return DAYS_SHORT[date.getDay()]
