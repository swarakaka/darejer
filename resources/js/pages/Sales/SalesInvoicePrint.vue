...
interface Invoice {
  ...
  salesperson: { name: string | null; username: string | null } | null
  lines: InvoiceLine[]
}
...
<template>
  ...
  >
    <header class="flex items-start justify-between border-b-2 border-black pb-4">
      ...
      <div class="text-end">
        <div class="text-[24px] font-bold tracking-wide">{{ __('SALES INVOICE') }}</div>
    ...
    <section class="mt-5 grid grid-cols-2 gap-6">
      <div>
        <div class="mb-1 text-[10px] font-semibold tracking-wide text-gray-600 uppercase">{{ __('Customer') }}</div>
        <div class="font-semibold">{{ localized(invoice.customer_account?.name) }}</div>
        <div v-if="invoice.customer_account?.legal_name" class="text-[11px]">
          {{ localized(invoice.customer_account?.legal_name) }}
        </div>
        <div v-if="invoice.customer_account?.code" class="text-[11px]">
          {{ __('Code') }}: {{ invoice.customer_account?.code }}
        </div>
        <div v-if="invoice.customer_account?.tax_id" class="text-[11px]">
          {{ __('Tax ID') }}: {{ invoice.customer_account?.tax_id }}
        </div>
        <div v-if="invoice.customer_account?.vat_id" class="text-[11px]">
          {{ __('VAT ID') }}: {{ invoice.customer_account?.vat_id }}
        </div>
        <div v-if="invoice.customer_account?.email" class="text-[11px]">{{ invoice.customer_account?.email }}</div>
        <div v-if="invoice.customer_account?.mobile" class="text-[11px]">
          <span dir="ltr" class="inline-block">{{ invoice.customer_account?.mobile }}</span>
        </div>
      </div>
      <div>
        ...
        <table class="w-full text-[11px]">
          <tbody>
            ...
            <tr v-if="invoice.contact">
              <td class="py-0.5 pe-3 text-gray-600">{{ __('Contact') }}</td>
              <td class="py-0.5">{{ invoice.contact?.full_name }}</td>
            </tr>
            <tr v-if="invoice.sales_order">
              <td class="py-0.5 pe-3 text-gray-600">{{ __('Sales Order') }}</td>
              <td class="py-0.5">{{ invoice.sales_order?.voucher_no }}</td>
            </tr>
            <tr v-if="invoice.delivery_note">
              <td class="py-0.5 pe-3 text-gray-600">{{ __('Delivery Note') }}</td>
              <td class="py-0.5">{{ invoice.delivery_note?.voucher_no }}</td>
            </tr>
            <tr v-if="invoice.salesperson">
              <td class="py-0.5 pe-3 text-gray-600">{{ __('Salesperson') }}</td>
              <td class="py-0.5">{{ invoice.salesperson?.name || invoice.salesperson?.username }}</td>
            </tr>
            <tr>
