...
<template>
  <div class="invoice mx-auto my-6 max-w-[210mm] bg-white p-10 text-[12px] text-black shadow print:my-0 print:shadow-none">
    ...
    <section class="mt-5 grid grid-cols-2 gap-6">
      <div>
        <div class="mb-1 text-[10px] font-semibold uppercase tracking-wide text-gray-600">{{ __('Customer') }}</div>
        <div class="font-semibold">{{ localized(invoice.customer_account?.name) }}</div>
        <div v-if="invoice.customer_account?.legal_name" class="text-[11px]">{{ localized(invoice.customer_account?.legal_name) }}</div>
        <div v-if="invoice.customer_account?.code" class="text-[11px]">{{ __('Code') }}: {{ invoice.customer_account?.code }}</div>
